        ...
        <div className="w-screen h-screen flex justify-center lg:flex-none lg:justify-start ">
            <div className="bg-white mt-[150px] max-w-[900px] w-[90vw] h-[570px] lg:h-[450px] lg:mx-10 relative rounded-md shadow-lg">
                <div className="z-10 w-full h-full absolute flex flex-col space-y-5 items-center justify-end pb-5 md:flex-row-reverse md:items-end md:justify-between px-5">
                    <div onClick={signOut} className="bg-slate-900 w-[100px] h-[40px] text-white text-xl rounded-md text-center flex items-center justify-center hover:bg-slate-700 cursor-pointer transition-transform ease-out duration-150 hover:scale-105">{t('profilePage.signout')}</div>
                ...
                <div className="w-full h-full absolute flex flex-col space-y-5 items-center justify-start pb-5 lg:flex-row lg:items-start lg:justify-between p-5">
                    ...
                    <div className="lg:pt-10 px-5 flex flex-col space-y-5 sm:space-y-1">
                        <div className="flex flex-row items-center">
                            ...
                            <img onClick={()=>{alert("ne bachka")}} className="w-[30px] h-[30px] z-10 cursor-pointer transition-transform ease-out duration-150 hover:scale-110" src="/assets/misc/edit.png"/>
                        </div>
                        <div className="flex flex-row items-start">
                            <h1 className="max-w-[545px] w-[80vw] h-[55px] font-robotoMono">{useremail}</h1>
                            <img onClick={()=>{alert("ne kazah li neshto")}} className="w-[20px] h-[20px] z-10 cursor-pointer transition-transform ease-out duration-150 hover:scale-110" src="/assets/misc/edit.png"/>
                        </div>
                    </div>
                </div>
            </div>
